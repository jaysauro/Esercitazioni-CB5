...
const formReg = document.forms.nuovi_registi;
const elementReg = formReg.elements;
...
const createCard = (data) => {
    ...
    const imgEl = ce("img");
    imgEl.className = "placeholderimg";
    ...
    surnameEl.className = "cardSurname";

    const dateEl = ce("p");
    dateEl.className = "cardBorn";
...
formReg.addEventListener("submit", (e) => {
    e.preventDefault();
    const data = {
        nome: element.nome_regista.value,
        cognome: element.cognome_regista.value,
    }
...
window.onload = GET(urlRegisti).then(res => res.map(regist => createCard(regist)));
